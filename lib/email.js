...
// A Resend Audience id. Optional: newer Resend accounts have a single default
// audience whose id we auto-discover from the API, so this only needs setting
// if you keep MULTIPLE audiences and want to target a specific one.
...
// Audience sync is available whenever we have an API key — the audience id is
// either configured or auto-discovered.
export function isAudienceEnabled() {
  return Boolean(RESEND_API_KEY);
}

// Resolve the Audience id to add contacts to: the configured one if set,
// otherwise the account's first (default) audience. Cached per instance so we
// only hit the list endpoint once. Returns null if the account has no audience.
let cachedAudienceId = RESEND_AUDIENCE_ID || null;
async function resolveAudienceId() {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Resend list audiences failed (${res.status}): ${detail}`);
  }
  const body = await res.json();
  const list = Array.isArray(body?.data) ? body.data : [];
  // Prefer an audience named like our waitlist (case-insensitive) so multiple
  // audiences resolve deterministically; otherwise fall back to the first.
  const preferred = list.find((a) => (a?.name || '').toLowerCase().includes('waitlist'));
  cachedAudienceId = (preferred || list[0])?.id || null;
...
// Returns { skipped: true } when no API key or no audience exists yet.
