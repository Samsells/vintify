...
// Deep-link a plan choice through signup. The CRM remembers the plan and, right
// after the account is created, auto-launches that plan's Stripe Checkout — so
// clicking "Start trial" / "Get lifetime" on the site lands the user in Stripe
// with one necessary account step in between. plan: 'monthly' | 'lifetime'.
export const signupUrl = (plan) => (plan ? `${SIGNUP_URL}?plan=${encodeURIComponent(plan)}` : SIGNUP_URL);
